...
ot.Client = (function (global) {
  ...
  // Client constructor
  function Client (revision, type) {
	this.type = type; // the ot type that is being used
  ...
  AwaitingConfirm.prototype.applyServer = function (client, operation) {
    ...
    //  to the client's
    //  current document)
    var pair = client.type.transformX(this.outstanding, operation);
  ...
  AwaitingWithBuffer.prototype.applyClient = function (client, operation) {
    ...
    var newBuffer = client.type.compose(this.buffer, operation);
  ...
  AwaitingWithBuffer.prototype.applyServer = function (client, operation) {
    ...
    //
    // * pair1[1]
    var pair1 = client.type.transformX(this.outstanding, operation);
    var pair2 = client.type.transformX(this.buffer, pair1[1]);
  ...
  AwaitingWithBuffer.prototype.serverAck = function (client) {
	...
	// Custom: Only sending 25 at a time so that we dont break the DynamoDB record limit
	var newOutstanding = this.buffer.splice(0, 25);
	  
    client.sendOperation(client.revision, newOutstanding);
    
    if(this.buffer.length > 0){
    	return new AwaitingWithBuffer(newOutstanding, this.buffer);
    }
    else{
