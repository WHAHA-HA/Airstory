            ...
            if (node.style[prop]) {
              if (properties.styleValue) {
                ...
                if (properties.styleValue instanceof RegExp) {
                  ...
                  //Custom: Changed it to a regex "text" rather than a string "match" which was returning a null error
                  if (properties.styleValue.test(node.style[prop].trim())) {
                    hasOneStyle = true;
                    break;
                  }
...
(function(wysihtml5) {
  ...
  function expandCaretToBlock(composer, insertingNodeName) {
    ...
    if (parent) {
      //Custom: Rangy was not selecting the full node when there were nodes within. For example, if a "p" element had a bullet list within,
      // the selection would fall within the "p" element and not contain it. This is my workaround.
      var realParent = $(parent).wrap('<span></span>')
      range = composer.selection.createRange();
      range.selectNodeContents(realParent.get(0));
    ...
    _initLineBreaking: function() {
      ...
      if (!this.config.useLineBreaks) {
        dom.observe(this.element, ["focus"], function() {
          //if (that.isEmpty()) {
            setTimeout(function() {
              //Custom: Check that it is still empty when the control returns to this timeout
              if (that.isEmpty()) {
