...
class ESConnection(object):
    ...
    def search(self, callback=None, **kwargs):
        source = json_encode(kwargs.pop('source', {
            "query": {
                "match_all": {}
            }
        }))
    ...
    @return_future
    def get(self, index, type, uid, callback=None, parameters=None):
        ...
        if callback:
            to_callback = to_dict_callback
            
        self.request_document(index, type, uid, callback=to_callback, parameters=parameters)
