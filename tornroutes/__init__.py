...
class route(object):
    ...
    def __call__(self, _handler):
        ...
        url_spec = tornado.web.url(self._uri, _handler, name=name)
        self._routes.append({'uri': self._uri, 'spec': url_spec})
        return _handler

    @classmethod
    def get_routes(self):
        #TODO: contrib back to project
        #Reverse sort so that there are not URI wildcard overlaps
        sorted_routes = sorted(self._routes, key=lambda k: k['uri'], reverse=True)
        
        routes = []
        for route in sorted_routes:
            logging.info(route['uri'])
...
def route_redirect(from_, to, name=None):
    url_spec = tornado.web.url(
        from_,
        tornado.web.RedirectHandler,
        dict(url=to),
        name=name)
    
    route._routes.append({'uri': from_, 'spec': url_spec})
